...
namespace UnrealSDK {
	...
	void __stdcall hkProcessEvent(UFunction* Function, void* Params, void* Result) {
		...
		auto lValueFStruct = FStruct{ Function, Params };
		if (gHookManager->HasHook(caller, Function) && !gHookManager->ProcessHooks(
			functionName, caller, Function, &lValueFStruct)) {
	...
	void hookGame() {
		...
		pProcessEvent = reinterpret_cast<tProcessEvent>(Util::GetSDKLogProcessEvent());
		Logging::LogF("[Internal] UObject::ProcessEvent() = 0x%p\n", pProcessEvent);

		pCallFunction = reinterpret_cast<tCallFunction>(Util::GetSDKLogCallFunction());
