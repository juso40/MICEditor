...
namespace Util {
	...
	int GetSDKLogProcessEvent() {
		std::ifstream infile(Settings::GetBinFile(L"python-sdk.log"));

		std::string line;
		std::string toFind = "[Internal] UObject::ProcessEvent() = ";
		while (std::getline(infile, line)) {
			if (line.find(toFind) != std::string::npos) return std::stoul(line.substr(toFind.length()), NULL, 0);
		}
		return 0;
	}

	int GetSDKLogCallFunction() {
		std::ifstream infile(Settings::GetBinFile(L"python-sdk.log"));

		std::string line;
		std::string toFind = "[Internal] UObject::CallFunction() = ";
		while (std::getline(infile, line)) {
			if (line.find(toFind) != std::string::npos) return std::stoul(line.substr(toFind.length()), NULL, 0);
		}
		return 0;
